...
void store_le32(uint8_t *output, uint32_t value) {
    output[0] = value & 0xFF; // least significant byte
    output[1] = (value >> 8) & 0xFF;
    output[2] = (value >> 16) & 0xFF;
    output[3] = (value >> 24) & 0xFF; // most significant byte
}
...
void md5_transform(MiniCrypt::md5_ctx& ctx, const uint8_t *input) {
    ...
    ctx.state[2] += c;
    ctx.state[3] += d;
    MiniCrypt::memset(x, 0, sizeof(x));
}
} // end namespace

void MiniCrypt::md5_init(md5_ctx& ctx) {
...
int MiniCrypt::md5_update(md5_ctx& ctx, const uint8_t *input, uint32_t size) {
    ...
    pad = 64 - index;
    if (size >= pad) {
        MiniCrypt::memcpy(&ctx.buffer[index], input, pad);
    ...
    MiniCrypt::memcpy(&ctx.buffer[index], &input[i], copy_len); // NOLINT(clang-analyzer-security.ArrayBound)
    return 0;
}

int MiniCrypt::md5_final(md5_ctx& ctx, uint8_t *out) {
    uint8_t bits[8];
    uint32_t index, pad;
    store_le32(&bits[0], ctx.count[0]);
    store_le32(&bits[4], ctx.count[1]);
    ...
    MiniCrypt::memset(&ctx, 0, sizeof(md5_ctx));
    return 0;
}

int MiniCrypt::md5_digest(const void *data, std::size_t size, uint8_t *out, const uint8_t *salt) {
    md5_ctx ctx;
    md5_init(ctx);
    if (salt)
