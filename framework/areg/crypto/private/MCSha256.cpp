...
void sha256_compress(MiniCrypt::sha256_ctx& ctx, const uint8_t block[64]) {
    uint32_t w[64];
...
int MiniCrypt::sha256_update(MiniCrypt::sha256_ctx& ctx, const uint8_t *in, size_t inlen) {
...
int MiniCrypt::sha256_final(MiniCrypt::sha256_ctx& ctx, uint8_t *out) {
    ...
    MiniCrypt::sha256_update(ctx, pad, pad_len + 8);
    for (int i = 0; i < 8; ++i)
        store_be32(out + ((ptrdiff_t)i * 4), ctx.state[i]);
    MiniCrypt::memset(&ctx, 0, sizeof(MiniCrypt::sha256_ctx));
    return 0;
}
